...
test("Claude plugin manifest and hooks config exist and reference the packaged entrypoint", () => {
...
test("Claude post-edit hook replays deterministic edit feedback through the packaged entrypoint", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "src-claude-hook-edit-"));
  ...
  const output = execFileSync(
    "node",
    ["./bin/claude-security-hook.js", "post-edit"],
    {
      cwd: repoRoot,
      encoding: "utf8",
      env: {
...
test("Claude pre-bash hook blocks staged high-severity checkpoint findings", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "src-claude-hook-git-"));
  const filePath = path.join(tempDir, "src/auth/login.js");
  ...
  const output = execFileSync(
    "node",
    ["./bin/claude-security-hook.js", "pre-bash"],
    {
      cwd: repoRoot,
      encoding: "utf8",
      env: {
